package dev.pebble.musicbridge.pixelplay.ui.theme
...
// LOCAL PATCH (not upstream). Upstream bundles Google Sans Flex (gflex_variable.ttf),
// which is not openly licensed, so this app ships Roboto Flex (OFL-1.1, see
// licenses/OFL-1.1-robotoflex.txt) instead. The ROND axis below is a Google Sans Flex
// roundedness setting; unregistered axes are ignored by other fonts, so on Roboto Flex
// it is simply a no-op and the family renders as a clean geometric grotesque.
